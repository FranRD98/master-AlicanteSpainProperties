...
function KT_setServerVariables() {
    ...
    if (!isset($_SERVER['PATH_TRANSLATED'])) {
        ...
        } elseif (isset($_SERVER['DOCUMENT_ROOT']) && isset($_SERVER['PHP_SELF'])) {
            $_SERVER['PATH_TRANSLATED'] = str_replace(['\\', '//'], ['/', '/'], $_SERVER['DOCUMENT_ROOT'] . $_SERVER['PHP_SELF']);
		...
		switch (strtolower($placeholderType)) {
			...
			if (isset($GLOBALS[$placeholderType])) {
				...
				if (is_resource($rs)) {
					...
					// Comprobar si el objeto es de tipo mysqli_result
					if ($rs instanceof mysqli_result) {
						// Obtener la fila actual de resultados como array asociativo
						$row = $rs->fetch_assoc();
						if ($row && isset($row[$placeholderName])) {
							$placeholderValue = $row[$placeholderName];
						} else {
							$placeholderValue = null;
						}
					} else {
						// Aquí podrías agregar soporte para otros tipos de objetos si es necesario
						$placeholderValue = null;
					}
				} else {
					break;
