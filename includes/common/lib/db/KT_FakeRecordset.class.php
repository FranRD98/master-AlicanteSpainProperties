...
class KT_FakeRecordset {
	...
	function getMySQLfakeRS() {
		...
		$delete_sql = 'DROP TEMPORARY TABLE IF EXISTS ' . $this->tmpTableName;
		...
		// Eliminar la tabla si existe antes de cualquier operación
		...
		foreach ($this->fields as $key => $value) {
			...
			$insert_sql .= $key . ', ';
			if (!is_array($value)) {
				$insert_values .= "'" . mysqli_real_escape_string($this->connection->connection, $value) . "', ";
			} else {
				$multiple = true;
			}
		}
		...
		if ($multiple) {
			...
			foreach ($this->fields as $key => $values) {
				...
				foreach ($values as $key => $value) {
					...
					$multiple_values[$i] .= "'" . mysqli_real_escape_string($this->connection->connection, $value) . "', ";
					$i++;
				}
			}
		...
		// Eliminar la tabla si existe antes de crear una nueva
		$this->connection->Execute($delete_sql);
		...
		// Eliminar la tabla temporal después de obtener los resultados
	...
	function getMySQL4fakeRS() {
		...
		for ($i = 0; $i < count($fields); $i++) {
			...
			foreach ($row as $colName => $value) {
				$select_sql .=  "'" . mysqli_real_escape_string($this->connection, $value) . "' AS " . $colName . ", ";
			}
