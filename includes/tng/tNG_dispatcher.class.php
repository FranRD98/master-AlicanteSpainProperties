...
class tNG_dispatcher {
	...
	public $UnivalErrors = array();
	
	public $UnivalProps = array();
	/**
	...
	function displayValidationRules() {
		global $_GET;
		$outRules = '';
		$outRules .= '<script src="/includes/tng/scripts/FormValidation.js" type="text/javascript"></script>' . "\r\n";
		
		if(isset($_GET['lang'])) {
			$outRules .= '<script src="/includes/tng/scripts/FormValidation.js.php?lang='.$_GET['lang'].'" type="text/javascript"></script>' . "\r\n";
		} else {
			$outRules .= '<script src="/includes/tng/scripts/FormValidation.js.php" type="text/javascript"></script>' . "\r\n";
		}
	...
	function displayFieldError($tableName, $fieldName, $cnt = 1) {
		...
		if ($ret != '') {
			return '<span class="help-block KT_field_error">'.$ret.'</span>'."\r\n";
		} else {
			return '';
		}
	}
	...
	function getErrorMsg() {
		...
		//$rethead .= '<script src="/includes/common/js/utility.js" type="text/javascript"></script>' . "\r\n";
		$ret = '';
		$txtContent = "";
		...
		if ($errorWasFound) {
			if ($ret_warning!='') {
				$ret .= '<!-- Warning Alert --><div class="alert alert-danger alert-dismissible alert-label-icon label-arrow fade show" role="alert"><i class="fa-regular fa-triangle-exclamation label-icon"></i> ' . $ret_warning . '<button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button></div>' . "\r\n";
				$txtContent .= "\r\n\r\nWarning:\r\n  " . $ret_warning;
			}
			if ($ret_user!='') {
				$ret .= '<!-- Danger Alert --><div class="alert alert-danger alert-dismissible alert-label-icon label-arrow fade show" role="alert"><i class="fa-regular fa-hexagon-exclamation label-icon"></i> <strong>' . KT_getResource('ERROR_LABEL','tNG') . '</strong> ' . $ret_user . '<button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button></div>' . "\r\n";
			...
			if ('DEVELOPMENT' == $GLOBALS['tNG_debug_mode']) {
				...
				$ret = $rethead . $ret;
				if ($ret_devel != '') {
					$ret .= '<!-- Warning Alert --><div class="alert alert-warning alert-dismissible alert-label-icon label-arrow fade show" role="alert"><i class="ri-alert-line label-icon"></i>'.$ret_devel.'<button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button></div>';
				}
				$tmp = tNG_log::getResult('html', '_'.$uniq);
				$ret .= '<!-- Warning Alert --><div class="alert alert-warning alert-dismissible alert-label-icon label-arrow fade show" role="alert"><i class="fa-regular fa-triangle-exclamation label-icon"></i> <strong>tNG Execution Trace - <a href="#" onclick="document.getElementById(\'KT_tngtrace_details_'. $uniq .'\').style.display=(document.getElementById(\'KT_tngtrace_details_'. $uniq .'\').style.display!=\'block\'?\'block\':\'none\'); return false;">VIEW</a></label></strong>'.$tmp.'<button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button></div>';
			}
	...
	function getLoginMsg() {
		...
		if ($show) {
			$info_resources = array('DENIED', 'MAXTRIES', 'ACCOUNT_EXPIRE');
			$info_key = KT_getRealValue("GET", "info");
			if ($info_key != "") {
				if (in_array($info_key, $info_resources)) {
					$ret = '<div class="alert alert-danger alert-dismissible alert-label-icon label-arrow fade show" role="alert">';
					$ret .= '<i class="fa-regular fa-hexagon-exclamation label-icon"></i><strong>'.KT_getResource('LOGIN_MESSAGE_LABEL','tNG').'</strong> ' . KT_getResource('LOGIN_MESSAGE__'.$info_key, 'tNG') . '';
          			$ret .= '<button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button>';
					$ret .= '</div>';
					return $ret;
				}
			}
			$info_resources = array('REG_ACTIVATE', 'REG_EMAIL', 'REG', 'ACTIVATED', 'FORGOT', 'LOGOUT');
			$info_key = KT_getRealValue("GET", "info");
			if ($info_key != "") {
				if (in_array($info_key, $info_resources)) {
					$ret = '<div class="alert alert-success alert-dismissible alert-label-icon label-arrow fade show" role="alert">';
					$ret .= '<i class="fa-regular fa-check label-icon"></i><strong>'.KT_getResource('LOGIN_MESSAGE_LABEL','tNG').'</strong> ' . KT_getResource('LOGIN_MESSAGE__'.$info_key, 'tNG') . '';
          			$ret .= '<button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button>';
					$ret .= '</div>';
					return $ret;
				}
