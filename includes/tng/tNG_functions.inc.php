...
function tNG_getRememberMePath() {
	...
	$absPath_running_script = str_replace("\\", "/", $_SERVER['SCRIPT_FILENAME']);
...
function tNG_deleteThumbnails($folder, $fileName, $md5) {
	...
	if ($fileName != '') {
        ...
        if (!preg_match('/\./', $idimg)) {
            @unlink(realpath($folder) . '/'.$idimg.'_sm.jpg');
            @unlink(realpath($folder) . '/'.$idimg.'_md.jpg');
            @unlink(realpath($folder) . '/'.$idimg.'_lg.jpg');
        } else {
