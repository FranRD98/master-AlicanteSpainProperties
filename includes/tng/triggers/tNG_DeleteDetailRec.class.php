	...
	function Execute() {
		...
		if (count($this->fileRenameRule) > 0 ) {
			for ($i=0; $i<count($fullFileName); $i++) {	
				...
				if (file_exists($fullFileNameFolder[$i] . $fullFileName[$i]) || preg_match('/http:\/\//', $fullFileName[$i])) {
					$delRet = @unlink($fullFileNameFolder[$i] . $fullFileName[$i]);
					$path_info = KT_pathinfo($fullFileNameFolder[$i] . $fullFileName[$i]);
					if(preg_match('/http:\/\//', $fullFileName[$i])) {
						
						$path = explode('/', $fullFileName[$i]);
						$filename=$path[count($path)-1];
						$filename= explode('.', $filename);
						$filename= $filename[0].'.'.$filename[1];
						
						//echo $filename;
						$this->deleteThumbnails($fullFileNameFolder[$i] . '/thumbnails/', $filename);
