...
?><?php
  ...
  array_push($aColumns, 'image_ban');


  array_push($aColumns, 'id_ban');
  ...
  while ( $aRow = mysqli_fetch_array( $rResult ) ) {
    $row = array();
    for ( $i=0 ; $i<count($aColumns) ; $i++ ) {
      if ( $aColumns[$i] == "image_ban" ) {
        /* Special output formatting for 'version' column */
        $row[] = '<img src="/media/images/banner/' . $aRow[ $aColumns[$i] ] . '?' . time() . '" alt="" class="banner-list-image">';
      }
      else {
