...
// ini_set("display_errors", 1);
  ...
  $aColumns = array();
  array_push($aColumns, 'category_' . $lang_adm . '_ct');

  array_push($aColumns, 'reporte_ct');
  array_push($aColumns, 'color_ct');
  ...
  while ( $aRow = mysqli_fetch_array( $rResult ) )
  {
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      if ( $aColumns[$i] == "color_ct" )
      {
        /* Special output formatting for 'version' column */
        $row[] =  '<div style="background: '.$aRow[ $aColumns[$i] ].'; display: block; width: 30px; height: 30px; border-radius: 4px; margin: 0 auto;"></div>';
      }
      else
      {
        /* General output */
