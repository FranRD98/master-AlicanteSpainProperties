...
$query_rsImagen = "SELECT * FROM news_files WHERE `news_files`.`id_fil` = '".$_GET['id']."'";
...
$query_rsImagenes = "DELETE FROM `news_files` WHERE `news_files`.`id_fil` = '".$_GET['id']."' LIMIT 1;";
$rsImagenes = mysqli_query($inmoconn,$query_rsImagenes) or die(mysqli_error());

if(unlink('../../media/files/news/'.$img)) {
	echo 'ok';
}
