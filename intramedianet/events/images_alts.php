...
$query_rsImg = sprintf("SELECT * FROM news_fotos WHERE id_img = %s", GetSQLValueString($colname_rsImg, "int"));
...
$set2 = '';
if(isset($_GET['destacada_img']) && $_GET['destacada_img'] != ''){
...
$set3 = '';
if(isset($_GET['mobile_img']) && $_GET['mobile_img'] != ''){
  $set3 .= ", mobile_img = 1";
}
else{
  $set3 .= ", mobile_img = 0";
...
$customTransaction->setSQL("UPDATE  `news_fotos` SET  ". $set. $set2 . $set3 ."  WHERE  `news_fotos`.`id_img` = ".$_GET['p']." LIMIT 1 ;");
// Add columns
foreach($languages as $value) {
...
$customTransaction->addColumn("mobile_img", "CHECKBOX_1_0_TYPE", "GET", "mobile_img", $row_rsImg['mobile_img']);

// End of custom transaction instance

$query_rsfoto = "SELECT id_img, noticia_img from news_fotos WHERE id_img = ".$_GET['p'];
...
<div class="bg-light">
    ...
    <div class="row">
      ...
      <div class="col-6">
        <div class="form-check form-switch form-switch-lg pt-2" dir="ltr">
          <input type="checkbox" name="mobile_img" id="mobile_img" value="1" class="form-check-input" <?php if (!(strcmp(KT_escapeAttribute($row_rscustom['mobile_img']),"1"))) {echo "checked";} ?> >

          <label class="form-check-label" for="mobile_img"><?php __('Móvil'); ?></label>
          <?php echo $tNGs->displayFieldError("custom", "mobile_img"); ?>
        </div>

        <div class="card p-2 bg-white mt-3">
          <small><i class="fa-regular fa-image fa-fw"></i> <strong><?php __('Foto vertical'); ?></strong></small> 
        </div>
    ...
    <input type="hidden" name="nw" id="nw" value="<?php echo $row_rsfoto['noticia_img'] ?>" />
