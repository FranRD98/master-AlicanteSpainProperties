...
WHERE  id_cli = '".simpleSanitize(($_GET['id']))."'
");
...
$html = preg_replace('/{NOMBRE}/', $user[0]['nombre_cli'] . ' ' . $user[0]['apellidos_cli'], $html);
$html = preg_replace('/{NIF}/', $user[0]['nie_cli'], $html);
...
try
{
    ...
    $html2pdf->Output('gdpr-' . clean($user[0]['nie_cli']) . '.pdf', 'I');
}
