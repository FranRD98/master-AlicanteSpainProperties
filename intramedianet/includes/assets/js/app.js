...
(function () {
	...
	if (navbarMenuElement) {
		var navbarMenuHTML = navbarMenuElement.innerHTML;
	} 
			...
			element.addEventListener("click", function () {
				...
				Toastify({
					...
					style: toastData.style == "style" ? {
						background: "linear-gradient(to right, #0AB39C, #252f38)"
					} : "",
				}).showToast();
			});
		});
	...
	function twoColumnMenuGenerate() {
		...
		if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
			...
			var twoColumMenu = document.getElementById("two-column-menu");
			if (twoColumMenu) {
