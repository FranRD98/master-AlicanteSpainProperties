...
?><?php
  ...
  if($sWhere != '') {
      $sWhere .= ' AND type_nws = 3';
  } else {
      $sWhere .= ' WHERE type_nws = 3';
  }
      ...
      else if ( $aColumns[$i] != ' ' )
      {
        if($aColumns[$i] == 'date_nws') {
            ...
            if(in_array($aColumns[$i], $aCamposIDMs)) {
                ...
                if ($aRow[ $aColumns[$i] ] != '') {
                    $row[] = '<a href="/intramedianet/landing/redirect.php?id_nws=' . $aRow['id_nws'] . '&lang=' . str_replace('_', '', $match[0]) . '" target="_blank" class="btn btn-light"><i class="fa-regular fa-link" aria-hidden="true"></i> ' . $aRow[ $aColumns[$i] ] . '</a>';
                } else {
                    $row[] = '';
                }
            } else {
