...
ORDER BY 
    name_".$lang_adm."_loc1, 
    name_".$lang_adm."_loc2, 
    name_".$lang_adm."_loc3, 
    name_".$lang_adm."_loc4 ASC
...
$query_rsProvincias = "
    SELECT 
        DISTINCT properties_loc1.name_".$lang_adm."_loc1,
        CASE WHEN properties_loc2.name_".$lang_adm."_loc2 IS NOT NULL THEN properties_loc2.name_".$lang_adm."_loc2 ELSE province1.name_".$lang_adm."_loc2  END AS name_".$lang_adm."_loc2,
        CASE WHEN properties_loc3.name_".$lang_adm."_loc3 IS NOT NULL THEN properties_loc3.name_".$lang_adm."_loc3 ELSE areas1.name_".$lang_adm."_loc3  END AS name_".$lang_adm."_loc3,
        CASE WHEN properties_loc4.name_".$lang_adm."_loc4 IS NOT NULL THEN properties_loc4.name_".$lang_adm."_loc4 ELSE towns.name_".$lang_adm."_loc4  END AS name_".$lang_adm."_loc4,
...
function addFields($tNG) {
  $tNG->addColumn("type_nws", "NUMERIC_TYPE", "EXPRESSION", "1");
  return $tNG->getError();
}
...
<body>
    ...
    <form method="post" id="form1" action="<?php echo KT_escapeAttribute(KT_getFullUri()); ?>" class="" novalidate enctype="multipart/form-data">
            ...
            <div class="col-lg-12">
                <div class="card position-relative">
                    <div class="card-header align-items-center d-flex card-header-fix">
                        <h4 class="card-title mb-0 flex-grow-1"><i class="fa-regular fa-newspaper"></i> <?php if (@$_GET['id_nws'] == "") { ?><?php echo NXT_getResource("Insert_FH"); ?><?php } else { ?><?php echo NXT_getResource("Update_FH"); ?><?php } ?> <?php echo __('Noticia'); ?></h4>
                        <div class="flex-shrink-0">
                            <?php if (!isset($_GET['id_nws']) || $_GET['id_nws'] == "") { ?>
                              ...
                              <ul class="nav nav-pills nav-custom nav-custom-light mb-3 d-md-none" role="tablist">
                                ...
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane <?php if($value == $language) { ?>active<?php } ?>" id="langcargo-<?php echo $value; ?>" role="tabpanel">
                                        ...
                                        <div class="form-group <?php if($tNGs->displayFieldError("news", "tags_".$value."_nws") != '') { ?>error<?php } ?>">
                                          <label for="tags_<?php echo $value; ?>_nws"><?php __('Etiquetas'); ?>:</label>
                                              <input type="text" name="tags_<?php echo $value; ?>_nws" id="tags_<?php echo $value; ?>_nws" value="<?php echo KT_escapeAttribute($row_rsnews['tags_'.$value.'_nws']); ?>" size="32" class="form-control tagsinput-<?php echo $value; ?>">
                            ...
                            <div class="card-body">
                                  ...
                                  ?>
                                      ...
                                      <div class="col-md-4 mt-2">
                                          <div class="form-check form-switch form-switch-lg pt-2" dir="ltr">
                                              <input type="checkbox" id="mtm_<?php echo $row_rsnews_categories['id_ct']; ?>" name="mtm_<?php echo $row_rsnews_categories['id_ct']; ?>" value="1" class="form-check-input" <?php if ($row_rsnews_categories['new'] != '') {?> checked<?php }?>>
                                              <label class="form-check-label" for="mtm_<?php echo $row_rsnews_categories['id_ct']; ?>"><?php echo $row_rsnews_categories["category_".$lang_adm."_ct"]; ?></label>
                                          </div>
                                      </div>

                                                    <?php //echo(getChildsCats($row_rsnews_categories['id_ct'],$row_rsnews_categories['parent_ctg'],$cnt1,$row_rsnews['id_nws'])); ?>
                                                    <?php
    ...
    <script src="/intramedianet/news/_js/news-form.js?id=<?php echo time(); ?>" type="text/javascript"></script>
    <script src="/intramedianet/includes/assets/_custom/vendor/tagsinput/bootstrap-tagsinput.js" type="text/javascript"></script>
    <script>
    ...
    <script>
      <?php foreach ($languages as $value): ?>
        ...
        $query_rsTags = "SELECT tags_".$value."_nws as tags FROM news WHERE tags_".$value."_nws != ''";
        ...
        if ($totalRows_rsTags > 0) {
          do {
            $tg = explode(',', $row_rsTags['tags']);
            foreach ($tg as $t) {
                if (!in_array($t, $tgs)) {
                    array_push($tgs, trim($t));
                }
            }
        ...
        sort($tgs);
        ?>
      var tags_<?php echo $value ?> = ['<?php echo implode("','", $tgs) ?>'];
      $('.tagsinput-<?php echo $value ?>').tagsinput({
        ...
        typeaheadjs: {
          name: 'tags_<?php echo $value ?>',
          source: substringMatcher(tags_<?php echo $value ?>)
        }
      });
      <?php endforeach ?>
    </script>
