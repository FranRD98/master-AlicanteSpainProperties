...
?><?php
  ...
  array_push($aColumns, 'date_usr');
  array_push($aColumns, 'cats');
  ...
  for ( $i=0 ; $i<count($aColumns) ; $i++ )
  {
    if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
    {
      ...
      if ($aColumns[$i] == 'cats') {
        ...
        $sWhere .= " (SELECT GROUP_CONCAT(' ', newsletter_categories.category_" .$lang_adm. "_cts) FROM newsletter_usr_cat INNER JOIN newsletter_categories ON newsletter_usr_cat.cat = newsletter_categories.id_cts WHERE newsletter_usr_cat.usr = id_usr ) LIKE '%".mysqli_real_escape_string($gaSql['link'],$_GET['sSearch_'.$i])."%'  ";
  ...
  $sQuery = "
  		...
  		`email_usr`,
  		`date_usr`,
  		(SELECT GROUP_CONCAT(' ', newsletter_categories.category_" .$lang_adm. "_cts) FROM newsletter_usr_cat INNER JOIN newsletter_categories ON newsletter_usr_cat.cat = newsletter_categories.id_cts WHERE newsletter_usr_cat.usr = id_usr ) as cats,
		id_usr
  		FROM   $sTable
  		$sWhere
  		$sOrder
