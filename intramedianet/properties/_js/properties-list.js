$(document).ready(function() {
    $(window).change('.search-inputs select', function(e) {
        var current = $(e.target);
        $(current).prev().val($(current).val()).trigger('keyup');
    });

    var oTable, asInitVals = [];

    $("thead input").keyup( function () {
        oTable.fnFilter( this.value, oTable.oApi._fnVisibleToColumnIndex(
            oTable.fnSettings(), $("thead input").index(this) ) );
    });

    $("thead input").each( function (i) {
        asInitVals[i] = this.value;
    });

    $("thead input").blur( function (i) {
        if ( this.value == "" )
        {
            this.value = asInitVals[$("thead input").index(this)];
        }
    });

    $("thead .search-clear").click( function (i) {
        $("thead input").val('');
        fnResetAllFilters()
    });

    function fnResetAllFilters() {
        var oSettings = oTable.fnSettings();
        for(iCol = 0; iCol < oSettings.aoPreSearchCols.length; iCol++) {
            oSettings.aoPreSearchCols[ iCol ].sSearch = '';
        }
        $('select').prop('selectedIndex',0);
        oTable.fnDraw();
    }

    var colDef = [
        {
            "targets": 0,
            "orderable": false,
            "searchable": false,
            "sClass": "imgprop"
        },
        {
            "targets": ocultarCols,
            "visible": false,
        },
        {
            "render": function ( data, type, row ) {
                var changeV = data == non ? 1: 0;
                var bntImage = data == non ? '<div class="text-center mt-1"><i class="fa-regular fa-xmark text-danger fs-4 fw-bolder"></i></div>': '<div class="text-center mt-1"><i class="fa-regular fa-check text-success fs-4 fw-bolder"></i></div>';
                return '<a href="properties-change.php?s=destacado_prop&v='+changeV+'&id_prop=' +  row[numCols] + '" class="update-status">'+bntImage+'</a>';
            },
            "targets": destacado_propCol
        },
        {
            "render": function ( data, type, row ) {
                var changeV = data == non ? 1: 0;
                var bntImage = data == non ? '<div class="text-center mt-1"><i class="fa-regular fa-xmark text-danger fs-4 fw-bolder"></i></div>': '<div class="text-center mt-1"><i class="fa-regular fa-check text-success fs-4 fw-bolder"></i></div>';
                return '<a href="properties-change.php?s=activado_prop&v='+changeV+'&id_prop=' +  row[numCols] + '" class="update-status">'+bntImage+'</a>';
            },
            "targets": activado_propCol
        },
        {
            "render": function ( data, type, row ) {
                var changeV = data == non ? 1: 0;
                var bntImage = data == non ? '<div class="text-center mt-1"><i class="fa-regular fa-xmark text-danger fs-4 fw-bolder"></i></div>': '<div class="text-center mt-1"><i class="fa-regular fa-check text-success fs-4 fw-bolder"></i></div>';
                return '<a href="properties-change.php?s=vendido_prop&v='+changeV+'&id_prop=' +  row[numCols] + '" class="update-status">'+bntImage+'</a>';
            },
            "targets": vendido_propCol
        }
    ];

    if(typeof extraColDef !== 'undefined' && extraColDef.length > 0) {
        colDef = colDef.concat(extraColDef);
    }


    colDef.push({
        "render": function ( data, type, row ) {
            btns = '<div class="dropdown d-inline-block w-50">';
                btns += '<button class="btn btn-soft-primary btn-sm dropdown w-100" type="button" data-bs-toggle="dropdown" aria-expanded="false">';
                    btns += '<i class="fa-regular fa-ellipsis align-middle"></i>';
                btns += '</button>';
                btns += '<ul class="dropdown-menu dropdown-menu-end">';
                    btns += '<li><a href="properties-form.php?id_prop=' + data + '&amp;KT_back=1" class="dropdown-item edit-item-btn"><i class="fa-regular fa-pencil align-bottom me-1"></i> ' + dtEditar + '</a></li>';
                    if (canDel == 1) {
                        btns += '<li><hr class="dropdown-divider"></li>';
                        btns += '<li><a href="properties-form.php?id_prop=' + data + '&amp;KT_back=1&amp;KT_Delete1=1" class="dropdown-item remove-item-btn text-danger delrow"><i class="fa-regular fa-trash-can me-1"></i> ' + dtEliminar + '</a></li>';
                    }
                btns += '</ul>';
            btns += '</div>';
            btns  += '&nbsp;<a href="javascript:;" data-id="' + row[numCols] + '" class="btn btn-soft-info btn-sm btn-print-prop"><i class="fa-regular fa-print"></i></a> ';
            var theID = row[numCols];
            if($.inArray(theID,theFavs) == -1) {
            btns  += '&nbsp;<a href="fav-add.php?favadm=' + row[numCols] + '" class="btn btn-primary btn-sm"><i class="fa-regular fa-star"></i></a> ';
            } else {
            btns  += '&nbsp;<a href="fav-del.php?favadm=' + row[numCols] + '" class="btn btn-danger btn-sm"><i class="fa-regular fa-star"></i></a> ';
            }
            return  btns;
        },
        "targets": numCols,
        "orderable": false,
        "searchable": false
    });

    oTable = $('#records-tables').dataTable({
        "sAjaxSource": "properties-data.php",
        dom: "<'row'<'col-sm-12 table-responsiveSCRLL' tr>><'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7'p>>lB",
        buttons: [
            {
                extend: 'colvis',
                columns: columVis,
                className: 'btn btn-soft-primary btn-sm w-100',
                text: '<i class="fa-regular fa-table"></i>',
                align: "button-right"
            }
        ],
        "order": [[ sortval, "asc" ]],
        "bSortCellsTop": true,
        "bStateSave": true,
        "fnInitComplete": function() {
            var oSettings = $('#records-tables').dataTable().fnSettings();
            var x= 0;
            for ( var i=0 ; i<oSettings.aoPreSearchCols.length ; i++ ){
                if (oSettings.aoColumns[i].bVisible == false) {
                    x++;
                    continue;
                }
                if(oSettings.aoPreSearchCols[i].sSearch.length>0){
                    $("thead input")[i-x].value = oSettings.aoPreSearchCols[i].sSearch;
                }
            }
            toggleScrollBarIcon();
            $(document).delegate('*[data-toggle="lightbox"]', 'click', function(e) {
                e.preventDefault();
                $(this).ekkoLightbox();
            });

            $.map( $('.search-inputs select'), function( select, i ) {
                var input = $(select).prev();
                if(input.val() != '') {
                    $(select).val(input.val());
                }
            });

            $('[data-toggle="tooltip"]').tooltip();

        },
        "columnDefs": colDef

    });

    $("#records-tables_length").appendTo("#col-1");
    $(".buttons-colvis").appendTo("#col-2");

});
