...
?><?php
  ...
  if ($actUsuarios == 1) {
    $aColumns = array( 'nombre_cli', 'apellidos_cli', 'email_cli', 'telefono_fijo_cli', 'telefono_movil_cli', 'captado_por2_cli', 'puntuacion_cli', 'status_cli', 'total', 'atendido_cli', 'atendido_por_cli', 'next_call_cli', 'fecha_alta_cli', 'id_cli' );
  } else {
    $aColumns = array( 'nombre_cli', 'apellidos_cli', 'email_cli', 'telefono_fijo_cli', 'telefono_movil_cli', 'captado_por2_cli', 'puntuacion_cli', 'status_cli', 'atendido_cli', 'atendido_por_cli', 'next_call_cli', 'fecha_alta_cli', 'id_cli' );
  }
...
/*
  ...
  for ( $i=0 ; $i<count($aColumns) ; $i++ )
  {
    ...
    if($sWhere != '') {
        $sWhere .= ' AND archived_cli = 1 ';
    } else {
        $sWhere .= ' WHERE archived_cli = 1 ';
    }
            ...
            if ($aColumns[$i] == 'nivel_usr') {
                ...
                if ($aColumns[$i] == 'nombre_cli') {
                  if($aRow[ $aColumns[$i] ]!==null)
                    $row[] = mb_convert_encoding($aRow[ $aColumns[$i] ], 'UTF-8', 'ISO-8859-1') . ' <br><span class="listdata text-nowrap" data-id="' . $aRow['id_cli'] . '"></span><br><a href="clients-form.php?id_cli=' . $aRow['id_cli'] . '&amp;KT_back=1" class="btn btn-success btn-sm w-100 mt-2"><i class="fa-regular fa-pencil align-bottom me-1"></i> ' . __('Editar', true) . '</a>';
                  else
                    $row[] = '';
                } else {
                    if ($aColumns[$i] == 'next_call_cli') {
