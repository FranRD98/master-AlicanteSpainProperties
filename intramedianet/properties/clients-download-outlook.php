...
?><?php
  ...
  if($sWhere != '') {
      $sWhere .= ' AND email_cli != \'\'';
  } else {
      $sWhere .= ' WHERE email_cli != \'\' ';
  }
  ...
  $sQuery = "
    ...
    CONCAT(nombre_cli, ' ', apellidos_cli) as nombre_cli,
     email_cli
    FROM   $sTable
    $sWhere

    $pnt $nom $apll $tel1 $tel2 $nie $pas $nac $sta $fech $fech2 $mail $skyp $direc $con $capt $hist $nots $capt2
      $st $ty $bd $bt $ref $prd $prh $loc4 $loc3 $loc2 $loc1 $op $or $or2 $op2
