...
?><?php
  ...
  /* DB table to use */
  $sTable = "properties_client_states";
    ...
    if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
    {
      ...
      if($aColumns[$i] == 'fecha_alta_cli') {
        ...
        if ($aColumns[$i] == 'inmueble_cons') {
            ...
            $sWhere .= "(SELECT referencia_clip FROM properties_cliperties WHERE id_stsp = inmueble_cons LIMIT 1) LIKE '%".mysqli_real_escape_string($gaSql['server'],$_GET['sSearch_'.$i])."%' ";
