...
?><?php
  ...
  $aColumns = array();
  array_push($aColumns, 'condition_' . $lang_adm . '_cond');

  array_push($aColumns, 'id_cond');

  /* Indexed column (used for fast and accurate table cardinality) */
  $sIndexColumn = "id_cond";

  /* DB table to use */
  $sTable = "properties_condition";
   ...
   * Get data to display
   */
  $campos = '`condition_' . $lang_adm . '_cond`, ';

  $sQuery = "
    SELECT SQL_CALC_FOUND_ROWS
      " . $campos . "
      id_cond
    FROM properties_condition
    $sWhere
    $sOrder
    $sLimit
  ";
  ...
  // echo "</pre><hr>";
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
          if ( $aColumns[$i] == "color_cond" || $aColumns[$i] == "text_color_cond" )
          {
            /* Special output formatting for 'version' column */
