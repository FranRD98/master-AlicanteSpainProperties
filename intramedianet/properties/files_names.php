...
$query_rsImg = sprintf("SELECT * FROM properties_files WHERE id_fil = %s", GetSQLValueString($colname_rsImg, "int"));
...
if($set!=''){
  $customQuery = "UPDATE  `properties_files` SET  ". $set."  WHERE  `properties_files`.`id_fil` = ".$_GET['p']." LIMIT 1 ;";
  $customTransaction->setSQL($customQuery);
...
<div class="bg-light">
    ...
    <div class="mb-2 <?php if($tNGs->displayFieldError("custom", "name_".$value."_fil") != '') { ?>has-error<?php } ?>">
        <div class="input-group">
            ...
            <?php } else { ?>
                <input type="text" class="form-control" name="name_<?php echo $value; ?>_fil" id="name_<?php echo $value; ?>_fil" value="<?php if(isset($row_rscustom['name_'.$value.'_fil'])) echo KT_escapeAttribute($row_rscustom['name_'.$value.'_fil']); ?>">
            <?php } ?>
