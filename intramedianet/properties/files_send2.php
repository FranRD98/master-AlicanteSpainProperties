...
$query_rsCliente = "SELECT * FROM properties_owner WHERE `id_pro` = '".$_GET['id_pro']."'";
...
$body  .= "<p><b>Cliente/Client:</b> " . $row_rsCliente['nombre_pro'] . " " . $row_rsCliente['apellidos_pro'] . "</p>";

for ($i=0; $i < count($ids); $i++) {

	$_GET['id'] =  $ids[$i];

    $query_rsFiles = "SELECT * FROM properties_owner_files WHERE id_fil = " . $ids[$i];
    ...
    do {
        $body  .= '<hr><a href="https://' . $_SERVER['HTTP_HOST'] . '/media/files/owners/' . $row_rsFiles['file_fil'] . '" target="_blank" style="display: inline-block; padding: 5px 10px 3px 10px; background: '.$mailColor.'; text-decoration: none; color: #fff; text-transform: uppercase; border-radius: 4px; font-size: 12px;">Descargar/download: ' . $row_rsFiles['file_fil'] . '</a>';
