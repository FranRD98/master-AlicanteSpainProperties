...
?><?php
   ...
   */
  $aColumns = array( 'nombre_usr', 'referencia_log', 'action_log', 'date_log' , 'id_log' );

  /* Indexed column (used for fast and accurate table cardinality) */
  $sIndexColumn = "id_log";

  /* DB table to use */
  $sTable = "properties_log";
  ...
  if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
  {
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
        if($aColumns[$i] == 'datein_log' || $aColumns[$i] == 'dateout_log') {
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      else if ( $aColumns[$i] != ' ' )
      {
        if($aColumns[$i] == 'date_log') {
          ...
          if ($aColumns[$i] == 'action_log') {
            switch ($aRow[ $aColumns[$i] ]) {
              case '1':
                $row[] = '<span class="badge bg-success">'.__('Añadido', true) . '</span>';
                break;
              case '2':
                $row[] = '<span class="badge bg-info">'.__('Editado', true) . '</span>';
                break;
              case '3':
                $row[] = '<span class="badge bg-warning">'.__('Editado: Bajada de precio', true) . '</span>';
                break;
              case '4':
                $row[] = '<span class="badge bg-warning">'.__('Editado: Subida de precio', true) . '</span>';
                break;
              case '5':
                $row[] = '<span class="badge bg-danger">'.__('Borrado', true) . '</span>';
                break;
            }
          } else {
            if ($aColumns[$i] == 'referencia_log') {
