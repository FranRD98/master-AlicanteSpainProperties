...
foreach ($ids as $key => $id) {
	...
	$query_rsImagen = "SELECT * FROM properties_images WHERE `properties_images`.`id_img` = '".$id."'";
    ...
    if(!preg_match('/https?:\/\//', $img)){
	    $filExtens = pathinfo("../../media/images/properties/".$img, PATHINFO_EXTENSION);
		...
		if ($filExtens == 'blm' || $filExtens == 'jpg' || $filExtens == 'jpeg' || $filExtens == 'gif' || $filExtens == 'JPG' || $filExtens == 'JPEG' || $filExtens == 'GIF' || $filExtens == 'png' || $filExtens == 'PNG') {
	        $zip->addFile("../../media/images/properties/".$img, $img) or die ("ERROR: Could not add file: $key");
	    }
	} else {
