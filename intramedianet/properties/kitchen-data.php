...
?><?php
  ...
  $aColumns = array();
  array_push($aColumns, 'kitchen_' . $lang_adm . '_kchn');

  array_push($aColumns, 'id_kchn');

  /* Indexed column (used for fast and accurate table cardinality) */
  $sIndexColumn = "id_kchn";

  /* DB table to use */
  $sTable = "properties_kitchen";
   ...
   */
  $campos = '';
  $campos .= '`kitchen_' . $lang_adm . '_kchn`, ';

  $sQuery = "
    SELECT SQL_CALC_FOUND_ROWS
      " . $campos . "
      id_kchn
    FROM properties_kitchen
    $sWhere
    $sOrder
    $sLimit
  ";
  ...
  // echo "</pre><hr>";
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
          if ( $aColumns[$i] == "color_kchn" || $aColumns[$i] == "text_color_kchn" )
          {
            /* Special output formatting for 'version' column */
