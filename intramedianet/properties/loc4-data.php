...
if ($mapeo == 1) {
  ...
  foreach ($languages as $value) {
    $campos .= 'loc4.name_' . $value . '_loc4, ';
  }

  if($sWhere == '') {
    $sWhere .= " WHERE    loc4.loc3_loc4 = '".$_GET['NxT_id_loc3']."' ";
  } else {
    $sWhere .= " AND    loc4.loc3_loc4 = '".$_GET['NxT_id_loc3']."' ";
  }
...
if ($mapeo == 1) {
  ...
  $sQuery = "
    ...
    FROM   $sTable WHERE    loc3_loc4 = '".$_GET['NxT_id_loc3']."'
  ";
  ...
  while ( $aRow = mysqli_fetch_array( $rResult ) )
  {
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      if ($aColumns[$i] != 'properties_loc4.name_'.$lang_adm.'_loc4') {
        $row[] = $aRow[ preg_replace('/loc4\./', '', $aColumns[$i]) ];
      } else {
