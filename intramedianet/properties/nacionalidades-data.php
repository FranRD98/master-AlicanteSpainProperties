...
?><?php
  ...
  $aColumns = array();
    array_push($aColumns, 'nacionalidad_en_ncld');
    array_push($aColumns, 'nacionalidad_es_ncld');





  array_push($aColumns, 'id_ncld');

  /* Indexed column (used for fast and accurate table cardinality) */
  $sIndexColumn = "id_ncld";

  /* DB table to use */
  $sTable = "nacionalidades";
  ...
  $sQuery = "
    SELECT SQL_CALC_FOUND_ROWS
      nacionalidad_en_ncld,
      nacionalidad_es_ncld,
      id_ncld
    FROM nacionalidades
    $sWhere
    $sOrder
  ...
  // echo "</pre><hr>";
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
          if ( $aColumns[$i] == "color_ncld" || $aColumns[$i] == "text_color_ncld" )
          {
            /* Special output formatting for 'version' column */
            $row[] =  '<div style="background: '.$aRow[ $aColumns[$i] ].'; display: block; width: 30px; height: 30px; border-radius: 4px; margin: 0 auto; border: 1px solid #000;"></div>';
          }
          else
          {
            /* General output */
