...
if (isset($_GET['id_fil'])) {
  $colname_rsFiles = $_GET['id_fil'];
}

$colname_rsFiles = mysqli_real_escape_string($inmoconn, $colname_rsFiles);

$query_rsFiles = sprintf("SELECT * FROM properties_owner_files WHERE owner_fil = %s ", GetSQLValueString($colname_rsFiles, "int"));
...
?>
 <?php if($totalRows_rsFiles >=  1) { ?>
      ...
      <div class="img-thumbnail pull-left">
        ...
        <p class="text-center">
          <?php if(isset($row_rsFiles['lang_fil']) && $row_rsFiles['lang_fil'] != '') { ?>
          ...
          <a href="ofiles_del.php" data-id="<?php echo $row_rsFiles['id_fil'] ?>" class="btn btn-danger btn-sm del-fil"><i class="fa-regular fa-trash-can"></i></a>
        </p>
      </div>
