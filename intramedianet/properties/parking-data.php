...
?><?php
  ...
  $aColumns = array();
  array_push($aColumns, 'parking_' . $lang_adm . '_prk');

  array_push($aColumns, 'id_prk');

  /* Indexed column (used for fast and accurate table cardinality) */
  $sIndexColumn = "id_prk";

  /* DB table to use */
  $sTable = "properties_parking";
   ...
   * Get data to display
   */
  $campos = '`parking_' . $lang_adm . '_prk`, ';

  $sQuery = "
    SELECT SQL_CALC_FOUND_ROWS
      " . $campos . "
      id_prk
    FROM properties_parking
    $sWhere
    $sOrder
    $sLimit
  ";
  ...
  // echo "</pre><hr>";
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
          if ( $aColumns[$i] == "color_prk" || $aColumns[$i] == "text_color_prk" )
          {
            /* Special output formatting for 'version' column */
