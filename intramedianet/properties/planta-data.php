...
?><?php
  ...
  $aColumns = array();
  array_push($aColumns, 'planta_' . $lang_adm . '_plnt');

  array_push($aColumns, 'id_plnt');

  /* Indexed column (used for fast and accurate table cardinality) */
  $sIndexColumn = "id_plnt";

  /* DB table to use */
  $sTable = "properties_planta";
   ...
   * Get data to display
   */
  $campos = '`planta_' . $lang_adm . '_plnt`, ';

  $sQuery = "
    SELECT SQL_CALC_FOUND_ROWS
      " . $campos . "
      id_plnt
    FROM properties_planta
    $sWhere
    $sOrder
    $sLimit
  ";
  ...
  // echo "</pre><hr>";
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
          if ( $aColumns[$i] == "color_plnt" || $aColumns[$i] == "text_color_plnt" )
          {
            /* Special output formatting for 'version' column */
