...
?><?php
   ...
   */
    ...
    $aColumns = array( 'types_' . $lang_adm . '_arc', 'id_arc' );
    ...
    if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
    {
      ...
      if($aColumns[$i] == 'fecha_alta_cli') {
        ...
        if ($aColumns[$i] == 'inmueble_cons') {
            ...
            $sWhere .= "(SELECT referencia_clip FROM properties_cliperties WHERE id_arcp = inmueble_cons LIMIT 1) LIKE '%".mysqli_real_escape_string($gaSql['link'],$_GET['sSearch_'.$i])."%' ";
