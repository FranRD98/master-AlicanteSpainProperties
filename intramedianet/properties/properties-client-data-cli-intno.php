...
?><?php
  ...
  $aColumns = array('image_img', 'referencia_prop', 'status_' .$lang_adm. '_sta', 'types_' .$lang_adm. '_typ', 'name_' .$lang_adm. '_loc3', 'name_' .$lang_adm. '_loc4', 'preci_reducidoo_prop', 'activado_prop', 'nombre_pro', 'telefono_fijo_pro', 'habitaciones_prop', 'aseos_prop', 'piscina_prop', 'parking_prop', 'id_prop', 'id_prop2', 'rate');
  ...
  do {
    if(isset($row_rsEmails['id_prop']))
    array_push($idemails, $row_rsEmails['id_prop']);
                  ...
                  if($aColumns[$i] == 'telefono_fijo_pro') {
                    ...
                    switch ($aColumns[$i]) {
                      ...
                      case 'name_' .$lang_adm. '_loc4':
                        ...
                        break;
                      case 'piscina_prop':
                        $sWhere .= "(SELECT pool_".$lang_adm."_pl FROM properties_pool WHERE id_pl = piscina_prop) LIKE '%".mysqli_real_escape_string($gaSql['link'],$_GET['sSearch_'.$i])."%' ";
                        break;
                      case 'parking_prop':
                        $sWhere .= "(SELECT parking_".$lang_adm."_prk FROM properties_parking WHERE id_prk = parking_prop) LIKE '%".mysqli_real_escape_string($gaSql['link'],$_GET['sSearch_'.$i])."%' ";
                        break;
...
if( isset($_GET['b_orientacion_cli']) && $_GET['b_orientacion_cli'] != '' ){
  ...
  // $retQRY = '';
  // if (isset($row_RS['ids']) && $row_RS['ids'] != '') {
 ...
 $retQRY = '';
  ...
  $sQuery = "
    SELECT SQL_CALC_FOUND_ROWS
      ...
      habitaciones_prop,
      aseos_prop,
      (SELECT rate FROM cli_prop_rate WHERE property = id_prop LIMIT 1) AS rate,
...
GROUP BY id_prop
  ...
  $sQuery = "
    ...
    FROM   $sTable WHERE id_prop != '' $retQRY
  ";
      ...
      else if ( $aColumns[$i] != ' ' )
      {
        if ($aColumns[$i] == 'image_img') {
          ...
          if (file_exists($_SERVER["DOCUMENT_ROOT"] . '/media/images/properties/thumbnails/'.$aResul['id_img'].'_md.jpg')) {
              $row[] = '<a href="/media/images/properties/thumbnails/'.$aResul['id_img'].'_md.jpg" data-toggle="lightbox"><img src="/media/images/properties/thumbnails/'.$aResul['id_img'].'_sm.jpg" alt="" style="max-height: 70px;"></a>';
          } else {
          ...
          if ($aColumns[$i] == 'preci_reducidoo_prop') {
              ...
              if ($aColumns[$i] == 'id_prop2') {
                if (in_array($aRow['id_prop'], $idemails)) {
                  $row[] = '1';
                } else {
                  $row[] = '0';
                }
                  ...
                  if ($aColumns[$i] == 'referencia_prop') {
                      $row[] = '<div class="text-center"><b>' . $aRow[ $aColumns[$i] ] . '</b><a href="_update-client-props.php?act=pur&sec=no&id_prop=' . $aRow['id_prop'] . '&id_cli=' . $_GET['id_cli'] . '" class="btn btn-success btn-sm w-100 btn-clnt" target="_blank"><i class="fa-regular fa-check"></i> ' . __('Incluir', true) . '</a></div>';
                      // <a href="properties-form.php?id_prop=' . $aRow['id_prop'] . '&KT_back=1" class="btn btn-success btn-sm w-100 mt-2" target="_blank"><i class="fa-regular fa-pencil"></i> ' . __('Editar', true) . '</a>
                  } else {
                      if ($aColumns[$i] == 'rate') {
                        if ($aRow[ $aColumns[$i] ] == 1 && $aRow[ $aColumns[$i] ] != NULL) {
                           $row['DT_RowClass'] = 'bg-soft-success';
                        }
                        if ($aRow[ $aColumns[$i] ] == 0 && $aRow[ $aColumns[$i] ] != NULL) {
