...
foreach ($ids as $id) {
    if ($id > 0) {
        ...
        WHERE id_prop = '".mysqli_real_escape_string($inmoconn,$id)."'";
...
//     $query_rsClient = "SELECT id_prop, referencia_prop FROM properties_properties
//     WHERE id_prop = '".mysqli_real_escape_string($_GET["q"])."'";
//     $rsClient = mysqli_query($query_rsClient, $inmoconn) or die(mysqli_error());
//     $row_rsClient = mysqli_fetch_assoc($rsClient);
//     $totalRows_rsClient = mysqli_num_rows($rsClient);
//     if( $totalRows_rsClient != 0){
//         do {
