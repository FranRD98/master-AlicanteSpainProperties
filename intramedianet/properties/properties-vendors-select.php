...
if( isset($_GET["q"]) && $_GET["q"] != "" ){
    ...
    $query_rsClient = "SELECT id_pro, nombre_pro, apellidos_pro FROM properties_owner
    WHERE nombre_pro LIKE '%".mysqli_real_escape_string($inmoconn,$_GET["q"])."%'
    OR apellidos_pro LIKE '%".mysqli_real_escape_string($inmoconn,$_GET["q"])."%'
    ORDER BY apellidos_pro ASC";
    ...
    if( $totalRows_rsClient != 0){
        do {
            $name = $row_rsClient["nombre_pro"].' '.$row_rsClient["apellidos_pro"];
            $results["results"][] = array(
                "id" => $row_rsClient["id_pro"],
                "text" => $name,
            );
