...
<?php if ($property[0]['id_prop'] > 0) { ?>
 ...
 <tr>
    <td style="padding: 20px 30px;">
        <h2 style="font-size: 20px; color: #000; text-align: center;">
        <?php if ($property[0]['titulo'] != ''): ?>
            <?php echo $property[0]['titulo']; ?>
        <?php else: ?>
            <?php echo $property[0]['type'] ?> - <?php echo $property[0]['sale'] ?> - <?php echo $property[0]['area'] ?> <?php if ($property[0]['area'] != $property[0]['town']): ?> - <?php echo $property[0]['town'] ?><?php endif ?>
        <?php endif ?>
        </h2>
        <a href="https://<?php echo $_SERVER['HTTP_HOST'] ?>/<?php echo propURL($property[0]['id_prop'], $langVal); ?>">
            <?php if (file_exists($_SERVER["DOCUMENT_ROOT"] . '/media/images/properties/thumbnails/'.$property[0]['id_img'] .'_md.jpg')) { ?>
                <img src="https://<?php echo $_SERVER['HTTP_HOST'] ?>/media/images/properties/thumbnails/<?php echo $property[0]['id_img'] ?>_md.jpg" alt="" style="width:100%; max-width: 100%; border-radius: 8px;">
            <?php } else { ?>
                <img src="https://<?php echo $_SERVER['HTTP_HOST'] ?>/media/images/website/no-image.png" alt="" style="width:100%; max-width: 100%; border-radius: 8px;">
            <?php } ?>
        </a>
        <p style="font-size: 16px; color: #333739; text-align: center; margin-bottom: 0;"><strong><?php echo $langStr["Ref "] ?>:</strong> <?php echo $property[0]['ref']; ?></p>

        <p style="font-size: 16px; color: #333739; text-align: center; margin-top: 0; margin-bottom: 0;"><strong>💶
             ...
             <?php if($property[0]['precio'] > 0) { ?>
                 <?php if($property[0]['old_precio'] > 0) { ?>
                     <del style="font-size: 14px; color: #f3001a; display: inline-block;"><?php echo number_format($property[0]['old_precio']*1,0, ',', '.'); ?>€</del>
                 <?php } ?>
                 <?php echo number_format($property[0]['precio']*1,0, ',', '.'); ?>€
             <?php } else{ ?>
                 <?php echo $langStr["Consultar"] ?>
             <?php } ?>
         </strong></p>
        <ul style="list-style: none; padding-left: 0; font-size: 16px; color: #555; line-height: 2em; text-align: center; margin: 0;">
            <?php if ($property[0]['m2_prop'] > 0): ?>
                <li>🏠 <?php echo number_format($property[0]['m2_prop']*1,0, ',', '.'); ?> m² <?php echo strtolower($langStr["Construidos"]); ?></li>
            <?php endif ?>
            <?php if ($property[0]['m2p_prop'] > 0): ?>
                <li>🌳 <?php echo number_format($property[0]['m2p_prop']*1,0, ',', '.'); ?> m² <?php echo strtolower($langStr["Parcela"]); ?></li>
            <?php endif ?>
            <?php if ($property[0]['habitaciones_prop'] > 0): ?>
                <li>🛏️ <?php echo number_format($property[0]['habitaciones_prop']*1,0, ',', '.'); ?> <?php echo strtolower($langStr["Habitaciones"]); ?></li>
            <?php endif ?>
            <?php if ($property[0]['aseos_prop'] > 0): ?>
                <li>🚿 <?php echo number_format($property[0]['aseos_prop']*1,0, ',', '.'); ?> <?php echo strtolower($langStr["Baños"]); ?></li>
            <?php endif ?>
            <?php if ($property[0]['piscina_prop'] != ''): ?>
                <li>🏊‍♂️ <?php echo $property[0]["piscina_prop"]; ?></li>
            <?php endif ?>
            <?php if ($property[0]['parking_prop'] != ''): ?>
                <li>🚗 <?php echo $property[0]["parking_prop"]; ?></li>
            <?php endif ?>
        </ul>
        <p style="font-size: 16px; color: #333739; text-align: center; margin-top: 0;">
        ...
        <p>
            <a href="https://<?php echo $_SERVER['HTTP_HOST'] ?>/<?php echo propURL($property[0]['id_prop'], $langVal); ?>" style="background-color: {COLOR}; color: #fff; padding: 15px 20px; text-decoration: none; border-radius: 223px; display: inline-block; border-radius: 23px; display: block; max-width: 100%; text-align: center; font-size: 16px;">🔍 <?php echo $langStr["Ver propiedad"]; ?></a>
        </p>
        <?php if ($show_rate == 1): ?>
        <p style="margin-top: 10px;">
            <a href="https://<?php echo $_SERVER['HTTP_HOST'] ?><?php echo $urlstart  ?>rate/?id_cli={CLIENT}&id_props={PROPS}#prop<?php echo $property[0]['id_prop']; ?>" style="font-size: 16px; color: #000;  text-align: center; display: block; text-decoration: none;">❤️ <?php echo $langStr["¿Te ha gustado esta casa? Valórala aquí"] ?></a>
        </p>
        <?php endif ?>
    </td>
 </tr>
 <tr>
    <td style="padding: 20px 30px;">
        <div style="display: block; border-top: 1px solid #dedede;"></div>
    </td>
 </tr>
<?php } ?>