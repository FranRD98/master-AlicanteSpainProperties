...
?><?php
  ...
  array_push($aCamposIDMs, 'title_' . $lang_adm . '_nws');

  array_push($aColumns, 'activate_nws');
  array_push($aColumns, 'destacado_nws');
      ...
      if ($aColumns[$i] == 'categoria_nws') {
        ...
        if($aColumns[$i] == 'date_nws') {
          $sWhere .= "DATE_FORMAT(`" . $aColumns[$i] . "`, '%d-%m-%Y %h:%i') LIKE '%".mysqli_real_escape_string($gaSql['link'],$_GET['sSearch_'.$i])."%' ";
            ...
            if($aColumns[$i] == 'activate_nws' || $aColumns[$i] == 'destacado_nws') {
  ...
  if($sWhere != '') {
      $sWhere .= ' AND type_nws = 4';
  } else {
      $sWhere .= ' WHERE type_nws = 4';
  }
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      else if ( $aColumns[$i] != ' ' )
      {
        if($aColumns[$i] == 'date_nws') {
            ...
            if(in_array($aColumns[$i], $aCamposIDMs)) {
                preg_match('/_(.*?)_/', $aColumns[$i], $match);
                if ($aRow[ $aColumns[$i] ] != '') {
                    $row[] = '<a href="/intramedianet/landing/redirect.php?id_nws=' . $aRow['id_nws'] . '&lang=' . str_replace('_', '', $match[0]) . '" target="_blank" class="btn btn-default"><i class="fa-regular fa-link" aria-hidden="true"></i> ' . $aRow[ $aColumns[$i] ] . '</a>';
                } else {
                    $row[] = '';
                }
            } else {
