...
?><?php
  ...
  array_push($aColumns, 'nombre_tms');

  array_push($aColumns, 'cargo_' . $lang_adm . '_tms');

  array_push($aColumns, 'telefono_tms');
  array_push($aColumns, 'email_tms');
  array_push($aColumns, 'activate_tms');


  array_push($aColumns, 'id_tms');
  ...
  if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
  {
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
        if ($aColumns[$i] == 'categoria_tms') {
          $sWhere .= " (SELECT category_" .$lang_adm. "_ct FROM news_categories WHERE id_ct = categoria_tms) LIKE '%".mysqli_real_escape_string( $gaSql['link'], $_GET['sSearch'] )."%' OR ";
        } else {
          if($aColumns[$i] == 'date_tms') {
  ...
  for ( $i=0 ; $i<count($aColumns) ; $i++ )
  {
    if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
    {
      ...
      if ($aColumns[$i] == 'categoria_tms') {
        ...
        $sWhere .= " (SELECT category_" .$lang_adm. "_ct FROM news_categories WHERE id_ct = categoria_tms) LIKE '%".mysqli_real_escape_string($gaSql['link'], $_GET['sSearch_'.$i])."%' ";
      } else {
        if($aColumns[$i] == 'date_tms') {
          $sWhere .= "DATE_FORMAT(`" . $aColumns[$i] . "`, '%d-%m-%Y %h:%i') LIKE '%".mysqli_real_escape_string($gaSql['link'], $_GET['sSearch_'.$i])."%' ";
        } else {
            if($aColumns[$i] == 'activate_tms') {
