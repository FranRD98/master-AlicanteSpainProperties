...
?><?php
  ...
  array_push($aColumns, 'id_tmpl');

  /* Indexed column (used for fast and accurate table cardinality) */
  $sIndexColumn = "id_tmpl";
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" )
      {
        if ($aColumns[$i] == 'categoria_nws') {
          $sWhere .= " (SELECT category_" .$lang_adm. "_ct FROM templates_categories WHERE id_ct = categoria_nws) LIKE '%".mysqli_real_escape_string( $gaSql['link'],$_GET['sSearch'] )."%' OR ";
        } else {
  ...
  for ( $i=0 ; $i<count($aColumns) ; $i++ )
  {
    if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
    {
      ...
      if ($aColumns[$i] == 'categoria_nws') {
        ...
        $sWhere .= " (SELECT category_" .$lang_adm. "_ct FROM templates_categories WHERE id_ct = categoria_nws) LIKE '%".mysqli_real_escape_string($gaSql['link'],$_GET['sSearch_'.$i])."%' ";
      } else {
        if($aColumns[$i] == 'date_nws') {
