        ...
        foreach ($languages as $lang) {
            ...
            if ($lang != $autotraduccion_from) {
                if ($row_rsTipos['types_' . $lang . '_typ'] == '') {
                    ...
                    $trad = $tr->translate(html_entity_decode($row_rsTipos['types_' . $autotraduccion_from . '_typ']),['target' => $langtrad,'source' => $autotraduccion_from]);
                    $query_rsUpdate = "UPDATE `properties_types` SET `types_" . $lang . "_typ` = '" . str_replace('& amp;', '&amp;', ucfirst(mysqli_real_escape_string($inmoconn, $trad['text']))) . "' WHERE `id_typ` = ".$row_rsTipos['id_typ']."";
        ...
        foreach ($languages as $lang) {
            ...
            if ($lang != $autotraduccion_from) {
                if ($row_rsFeatures['feature_' . $lang . '_feat'] == '') {
                    ...
                    $trad = $tr->translate(html_entity_decode($row_rsFeatures['feature_' . $autotraduccion_from . '_feat']),['target' => $langtrad,'source' => $autotraduccion_from]);
                    $query_rsUpdate = "UPDATE `properties_features_priv` SET `feature_" . $lang . "_feat` = '" . str_replace('& amp;', '&amp;', ucfirst(mysqli_real_escape_string($inmoconn, $trad['text']))) . "' WHERE `id_feat` = ".$row_rsFeatures['id_feat']."";
                    $rsUpdate = mysqli_query($inmoconn,$query_rsUpdate) or die(mysqli_error());
                }
            }
        }
            ...
            foreach ($languages as $lang) {
                ...
                if ($lang != $autotraduccion_from) {
                    if ($row_rsInmuebles['descripcion_' . $lang . '_prop'] == '') {
                        ...
                        $trad = $tr->translate(html_entity_decode($row_rsInmuebles['descripcion_' . $autotraduccion_from . '_prop']),['target' => $langtrad,'source' => $autotraduccion_from]);
                        $query_rsUpdate = "UPDATE `properties_properties` SET `descripcion_" . $lang . "_prop` = '" . str_replace('& amp;', '&amp;', ucfirst(mysqli_real_escape_string($inmoconn,$trad['text']))) . "' WHERE `id_prop` = ".$row_rsInmuebles['id_prop']."";
                        $rsUpdate = mysqli_query($inmoconn,$query_rsUpdate) or die(mysqli_error());
                    }
                }
            }
