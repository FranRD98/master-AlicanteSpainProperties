...
?><?php
   ...
   */
  $aColumns = array( 'nivel_usr', 'nombre_usr', 'email_usr', 'activar_usr', 'registrationdate_usr', 'id_usr' );
  ...
  if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
  {
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
        if($aColumns[$i] == 'registrationdate_usr') {
          $sWhere .= "DATE_FORMAT(`registrationdate_usr`, '%d-%m-%Y %h:%i') LIKE '%".mysqli_real_escape_string($gaSql['link'],$_GET['sSearch'])."%' OR ";
        }
        else {
            ...
            if ($aColumns[$i] == 'nivel_usr') {
                        ...
                        if (preg_match('/'.strtolower($_GET['sSearch']).'/', strtolower(__('Empleado', true)))) {
  ...
  if($sWhere != '') {
      $sWhere .= ' AND nivel_usr != 10 AND id_usr != 1 AND nivel_usr != 1 AND id_usr != 0 AND id_usr != 1';
  } else {
      $sWhere .= ' WHERE nivel_usr != 10 AND id_usr != 1 AND nivel_usr != 1 AND id_usr != 0 AND id_usr != 1';
  }
  ...
  $sQuery = "
    ...
    FROM   $sTable  WHERE nivel_usr != 10 AND id_usr != 1
  ";
