$(document).ready(function() {
    ...
    $('#tipo_xml_sel').change(function(e) {

        $('#tipo_xml').val($(this).val()).trigger('keyup');

    });

    $('#activate_xml_sel').change(function(e) {

        $('#activate_xml').val($(this).val()).trigger('keyup');
    ...
    oTable = $('#records-tables').dataTable({
        "sAjaxSource": "proveedores-data.php",
        "bSortCellsTop": true,
        "fnInitComplete": function() {
            ...
            if ($('#tipo_xml').val() != '') {
                ...
                $('#tipo_xml_sel').val($('#tipo_xml').val() );

            }

            if ($('#activate_xml').val() != '') {

                $('#activate_xml_sel').val($('#activate_xml').val() );
                ...
                "mRender": function (data, type, full) {
                    btns = '<div class="dropdown d-inline-block w-100">';
                        ...
                        btns += '<ul class="dropdown-menu dropdown-menu-end">';
                            btns += '<li><a href="proveedores-form.php?id_xml=' + data + '&amp;KT_back=1" class="dropdown-item edit-item-btn"><i class="fa-regular fa-pencil align-bottom me-1"></i> ' + dtEditar + '</a></li>';
                            if (canDel == 1) {
                                btns += '<li><hr class="dropdown-divider"></li>';
                                btns += '<li><a href="proveedores-form.php?id_xml=' + data + '&amp;KT_back=1&amp;KT_Delete1=1" class="dropdown-item remove-item-btn text-danger delrow"><i class="fa-regular fa-trash-can me-1"></i> ' + dtEliminar + '</a></li>';
                            }
                        btns += '</ul>';
                    btns += '</div>';
                        return  btns;
