...
?><?php
  ...
  array_push($aColumns, "name_exp");
  array_push($aColumns, "uid_exp");

  array_push($aColumns, 'id_exp');
  ...
  while ( $aRow = mysqli_fetch_array( $rResult ) )
  {
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      {
        if ($i == 3) {
          $row[] = mb_convert_encoding($aRow[ 3 ], 'UTF-8', 'ISO-8859-1');
        } else {
          if ($i == 0) {
            $row[] = mb_convert_encoding($aRow[ 0 ], 'UTF-8', 'ISO-8859-1');
          } else {
            $row[] = mb_convert_encoding($aRow[ preg_replace('/features\./', '', $aColumns[$i]) ], 'UTF-8', 'ISO-8859-1');
          }
        }
      }
    }
