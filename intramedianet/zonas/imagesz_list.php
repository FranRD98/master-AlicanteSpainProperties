...
if (isset($_GET['id_ct'])) {
  $colname_rsImagenes = $_GET['id_ct'];
}

$colname_rsImagenes = mysqli_real_escape_string($inmoconn, $colname_rsImagenes);

$query_rsImagenes = sprintf("SELECT * FROM zonas_images WHERE zona_img = %s ORDER BY order_img", GetSQLValueString($colname_rsImagenes, "int"));
...
?>
 <?php if($totalRows_rsImagenes >=  1) { ?>
                      ...
                      <div class="img-thumbnail pull-left">
                        <?php echo showThumbnail($row_rsImagenes['image_img'], '/media/images/zonas/', 150, 100); ?>
