...
$news = getRecords("
    ...
    SELECT news.id_nws,
        ...
        (SELECT imagen_img FROM news_fotos WHERE noticia_img = id_nws ORDER BY IF(destacada_img = 1, 0, 1), orden_img LIMIT 1) AS img,
        (SELECT alt_".$lang."_img  FROM news_fotos WHERE noticia_img = id_nws ORDER BY IF(destacada_img = 1, 0, 1), orden_img LIMIT 1) AS alt

    FROM news

    WHERE news.title_".$lang."_nws  != '' AND news.content_".$lang."_nws != '' AND type_nws = 6

    AND id_nws = '" . simpleSanitize(($_GET['ciu'])) . "'
...
$videos = getRecords("
    ...
    SELECT video_vid FROM news_videos WHERE news_vid = '" . simpleSanitize(($_GET['ciu'])) . "' ORDER BY order_vid

");

$smarty->assign("videos", $videos);

$provID = explode(',', $news[0]['zonas_nws']);
$i = 0;
...
if ($lang == $language) {
     $url = 'http://' . $_SERVER['HTTP_HOST'] . '/' . slug($news[0]['titulo']) . '.html';
} else {
    $url = 'http://' . $_SERVER['HTTP_HOST'] . $urlStart . '' . slug($news[0]['titulo']) . '.html';
}
...
$images = getRecords("
    ...
    SELECT imagen_img as image_img, alt_".$lang."_img as alt FROM news_fotos WHERE noticia_img = '".simpleSanitize(($_GET['ciu']))."' AND destacada_img != 1 ORDER BY orden_img
...
preg_match_all('/{image-left}|{image-right}|{image-pan}/', (string)$news[0]['contenido'], $matches);
...
if ($news[0]['quick_location_nws'] != '') {
    $whereSQL .= "AND (CASE WHEN properties_loc4.id_loc4 IS NOT NULL THEN properties_loc4.id_loc4 ELSE towns.id_loc4  END) = '".$news[0]['quick_location_nws']."'";
}

if ($news[0]['quick_town_nws'] != '') {
    $whereSQL .= "AND (CASE WHEN properties_loc3.id_loc3 IS NOT NULL THEN properties_loc3.id_loc3 ELSE areas1.id_loc3  END) = '".$news[0]['quick_town_nws']."'";
}

if ($news[0]['quick_type_nws'] != '') {
    $whereSQL .= "AND (CASE WHEN properties_types.id_typ IS NOT NULL THEN properties_types.id_typ ELSE types.id_typ END) = '".$news[0]['quick_type_nws']."'";
}

if ($news[0]['quick_status_nws'] != '') {
    $whereSQL .= "AND (operacion_prop) = '".$news[0]['quick_status_nws']."'";
}
...
foreach ($properties as $key => $value) {
    ...
    AND property = '".stripslashes(mysqli_real_escape_string($inmoconn,$properties[$key]['id_prop']))."'
    ");
...
if ($newsURLs[0]['titulo_ca'] != '' || $newsURLs[0]['titulo_ca'] != '') {
    if ($newsURLs[0]['titulo_ca'] != '') {
        $urlCA = 'https://' . $_SERVER['HTTP_HOST'] . '/ca/' . slug($newsURLs[0]['titulo_ca']) . '/' . slug($newsURLS2[0]['titulo_ca']) . '.html';
    } else {
        $urlCA = 'https://' . $_SERVER['HTTP_HOST'] . '/ca/' . slug($newsURLs[0]['titulo_ca']) . '/' . slug($newsURLS2[0]['titulo_ca']) . '.html';
    }
} else {
...
if ($newsURLs[0]['titulo_pl'] != '' || $newsURLs[0]['titulo_pl'] != '') {
    if ($newsURLs[0]['titulo_pl'] != '') {
        $urlPL = 'https://' . $_SERVER['HTTP_HOST'] . '/pl/' . slug($newsURLs[0]['titulo_pl']) . '/' . slug($newsURLS2[0]['titulo_pl']) . '.html';
    } else {
        $urlPL = 'https://' . $_SERVER['HTTP_HOST'] . '/pl/' . slug($newsURLs[0]['titulo_pl']) . '/' . slug($newsURLS2[0]['titulo_pl']) . '.html';
    }
} else {
