...
$features = getRecords("
    ...
    SELECT CASE WHEN properties_features.feature_".$langVal."_feat IS NOT NULL THEN properties_features.feature_".$langVal."_feat ELSE features.feature_".$langVal."_feat  END AS feat
    ...
    WHERE properties_property_feature.property = '".simpleSanitize(($idVal))."' ORDER BY properties_features.order_feat ASC");

$featuresXML = getRecords("

    SELECT CASE WHEN properties_features_priv.feature_".$langVal."_feat IS NOT NULL THEN properties_features_priv.feature_".$langVal."_feat ELSE features.feature_".$langVal."_feat  END AS feat
    ...
    WHERE properties_property_feature_priv.property = '".simpleSanitize(($idVal))."' ORDER BY properties_features_priv.order_feat ASC");

$features = array_merge($features, $featuresXML);


if ($property[0]['m2_prop'] > 0) {
    array_unshift($features, array('feat' => $langStr["Construidos"] . ': ' . number_format($property[0]['m2_prop'],0, ',', '.') . ' m<sup>2</sup>'));
}
if ($property[0]['m2_balcon_prop'] > 0) {
    array_unshift($features, array('feat' => $langStr["Terraza"] . ': ' . number_format($property[0]['m2_balcon_prop'],0, ',', '.') . ' m<sup>2</sup>'));
}
if ($property[0]['m2p_prop'] > 0) {
    array_unshift($features, array('feat' => $langStr["Parcela"] . ': ' . number_format($property[0]['m2p_prop'],0, ',', '.') . ' m<sup>2</sup>'));
}
if ($property[0]['distance_golf_prop'] > 0) {
      if ($property[0]['distance_golf_med_prop'] == 'Km') {
          $med = $langStr["km "];
      } else {
          if ($property[0]['distance_golf_med_prop'] == 'Mts') {
              $med = $langStr["mts "];
          } else {
              $med = $langStr["Mins "];
          }
      }
      array_unshift($features, array('feat' => $langStr["Distancia al campo de golf"] . ': ' . number_format($property[0]['distance_golf_prop'],0, ',', '.') . ' ' . $med));
}
if ($property[0]['distance_amenities_prop'] > 0) {
      if ($property[0]['distance_amenities_med_prop'] == 'Km') {
          $med = $langStr["km "];
      } else {
          if ($property[0]['distance_amenities_med_prop'] == 'Mts') {
              $med = $langStr["mts "];
          } else {
              $med = $langStr["Mins "];
          }
      }
      array_unshift($features, array('feat' => $langStr["Distancia a entretenimientos"] . ': ' . number_format($property[0]['distance_amenities_prop'],0, ',', '.') . ' ' . $med));
}
if ($property[0]['distance_airport_prop'] > 0) {
      if ($property[0]['distance_airport_med_prop'] == 'Km') {
          $med = $langStr["km "];
      } else {
          if ($property[0]['distance_airport_med_prop'] == 'Mts') {
              $med = $langStr["mts "];
          } else {
              $med = $langStr["Mins "];
          }
      }
      array_unshift($features, array('feat' => $langStr["Distancia al aereopuerto"] . ': ' . number_format($property[0]['distance_airport_prop'],0, ',', '.') . ' ' . $med));
}
if ($property[0]['distance_beach_prop'] > 0) {
      if ($property[0]['distance_beach_med_prop'] == 'Km') {
          $med = $langStr["km "];
      } else {
          if ($property[0]['distance_beach_med_prop'] == 'Mts') {
              $med = $langStr["mts "];
          } else {
              $med = $langStr["Mins "];
          }
      }
      array_unshift($features, array('feat' => $langStr["Distancia a la playa"] . ': ' . number_format($property[0]['distance_beach_prop'],0, ',', '.') . ' ' . $med));
}
if ($property[0]['orientacion_prop'] == 'o-n') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-n"]));
}
if ($property[0]['orientacion_prop'] == 'o-ne') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-ne"]));
}
if ($property[0]['orientacion_prop'] == 'o-e') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-e"]));
}
if ($property[0]['orientacion_prop'] == 'o-se') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-se"]));
}
if ($property[0]['orientacion_prop'] == 'o-s') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-s"]));
}
if ($property[0]['orientacion_prop'] == 'o-so') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-so"]));
}
if ($property[0]['orientacion_prop'] == 'o-o') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-o"]));
}
if ($property[0]['orientacion_prop'] == 'o-no') {
      array_unshift($features, array('feat' => $langStr["Orientación"] . ': ' . $langStr["o-no"]));
}
if ($property[0]['energia_prop'] != '') {
    array_unshift($features, array('feat' => $langStr["Calificación energética"] . ': ' . $property[0]['energia_prop']));
}
if ($property[0]['construccion_prop'] != '') {
    array_unshift($features, array('feat' => $langStr["Año de construcción"] . ': ' . $property[0]['construccion_prop']));
}
if ($property[0]['aseos2_prop'] > 0) {
    array_unshift($features, array('feat' => $langStr["Aseos"] . ': ' . number_format($property[0]['aseos2_prop'],0, ',', '.')));
}
if ($property[0]['aseos_prop'] > 0) {
    array_unshift($features, array('feat' => $langStr["Baños"] . ': ' . number_format($property[0]['aseos_prop'],0, ',', '.')));
}
if ($property[0]['habitaciones_prop'] > 0) {
    array_unshift($features, array('feat' => $langStr["Habitaciones"] . ': ' . number_format($property[0]['habitaciones_prop'],0, ',', '.')));
}
