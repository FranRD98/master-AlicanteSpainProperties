...
$active = (isset($tokens[2]) && $tokens[2] == '')?' active':'';
$menuCats .= '<a href="'.$urlStart.''.$urlStr['news']['url'].'/" class="list-group-item' . $active . '">'.$langStr['Todos'].'</a>';

foreach ($categories as $value) {

    $active = (isset($tokens[2]) && $tokens[2] == $value['id_ct'])?' active':'';
...
$http_referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'default_url';
