...
if (isset($_GET['date'])) {
    if (isset($_SESSION['formdate']) && $_SESSION['formdate'] != $_GET['date']) {
...
if (isset($_GET['features']) && $_GET['features'][0] != '') {
    ...
    $rsfeats = mysqli_query($inmoconn, $query_rsfeats) or die(mysqli_error($inmoconn));
...
if ((isset($_GET['ter'])) && ($_GET['ter'] != '')) {
    $searchTerm = trim($_GET['ter']);
    $terms = explode(' ', $searchTerm);
    $searchTerm = mysqli_real_escape_string($inmoconn, simpleSanitize($searchTerm));
    $titleNewsSearch = $langStr["Resultado de busqueda"] . ' "' . $searchTerm . '"';
    $searchTerm = implode(" ", array_map(fn($word) => "+" . $word, explode(" ", $searchTerm)));
    ...
    if (!empty($terms)) {
        ...
        foreach ($terms as $searchTerm) {
            $count++;
            if ($count == count($terms)) $fin = '';
            $searchFull .=  "(MATCH(properties_properties.descripcion_" . $lang . "_prop, properties_properties.keywords_" . $lang . "_prop, properties_properties.titulo_" . $lang . "_prop, properties_properties.title_" . $lang . "_prop) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_loc1.name_" . $lang . "_loc1) AGAINST (' $searchTerm ' IN BOOLEAN MODE) 
                            OR MATCH(province1.name_" . $lang . "_loc2) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_loc2.name_" . $lang . "_loc2) AGAINST (' $searchTerm ' IN BOOLEAN MODE) 
                            OR MATCH(areas1.name_" . $lang . "_loc3) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_loc3.name_" . $lang . "_loc3) AGAINST (' $searchTerm ' IN BOOLEAN MODE) 
                            OR MATCH(towns.name_" . $lang . "_loc4) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_loc4.name_" . $lang . "_loc4) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(types.types_" . $lang . "_typ) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_types.types_" . $lang . "_typ) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_status.status_" . $lang . "_sta) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_coast.coast_" . $lang . "_cst) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
                            OR MATCH(properties_features.feature_" . $lang . "_feat) AGAINST (' $searchTerm ' IN BOOLEAN MODE) ) " . $fin . " ";
        }
        $searchFull .= ")";
    }
    ...
    $searchFull = "
        AND (
        MATCH(properties_properties.descripcion_" . $lang . "_prop, properties_properties.keywords_" . $lang . "_prop, properties_properties.titulo_" . $lang . "_prop, properties_properties.title_" . $lang . "_prop) AGAINST (' $searchTerm ' IN BOOLEAN MODE) 
        OR MATCH(properties_loc1.name_" . $lang . "_loc1) AGAINST (' $searchTerm ' IN BOOLEAN MODE) 
        OR MATCH(province1.name_" . $lang . "_loc2) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
        OR MATCH(properties_loc2.name_" . $lang . "_loc2) AGAINST (' $searchTerm ' IN BOOLEAN MODE) 
        OR MATCH(properties_loc3.name_" . $lang . "_loc3) AGAINST (' $searchTerm ' IN BOOLEAN MODE) 
        ...
        OR MATCH(properties_status.status_" . $lang . "_sta) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
        OR MATCH(properties_coast.coast_" . $lang . "_cst) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
        OR MATCH(properties_features.feature_" . $lang . "_feat) AGAINST (' $searchTerm ' IN BOOLEAN MODE)
        )";
    */

    $tableFeature = '
        LEFT OUTER JOIN properties_property_tag ON properties_properties.id_prop = properties_property_tag.property
        LEFT OUTER JOIN properties_property_feature ON properties_properties.id_prop = properties_property_feature.property
        LEFT OUTER JOIN properties_features ON properties_features.id_feat = properties_property_feature.feature
    ';
}
...
$query_pro = "
    SELECT
        ...
        CASE WHEN areas1.name_" . $lang . "_loc3 IS NOT NULL THEN properties_loc3.name_" . $lang . "_loc3 ELSE areas1.name_" . $lang . "_loc3  END AS area,
    ...
    FROM 
        ...
        LEFT OUTER JOIN properties_coast ON properties_loc3.coast_loc3 = properties_coast.id_cst
        $tableTags
        $tableFeature
    WHERE  
        $secc
...
$properties = getRecords($query_pro);
...
foreach ($properties as $key => $value) {
    ...
    if (isset($_SESSION['kt_login_id'])) {
        ...
        WHERE user = '" . stripslashes(($_SESSION['kt_login_id'])) . "'
        AND property = '" . stripslashes(($properties[$key]['id_prop'])) . "'
        ");
    }
...
$query = "
    SELECT
        ...
        " . $tableFeature . "
    WHERE 
        lat_long_gp_prop != '' AND 
        lat_long_gp_prop != '0,0' AND 
        procesada_img = 1 AND 
        activado_prop = 1 AND 
        alquilado_prop = 0 AND 
        vendido_prop = 0 AND 
        force_hide_prop != 1
        $pdh $p $lo $rf $ar $lc $typ $bd $bt $rp $nw $cs $sw $ep $po $cos $inl $rps $st $ven $alq $res $asc $m2ut $m2pl $ctr $lopr $loct $lozn $or $golf $prds $prhs  $coast
        $searchFull    
    GROUP BY id_prop
";
...
$viewpropsquery = 'WHERE id_prop = 0 AND procesada_img = 1 AND activado_prop = 1 AND alquilado_prop = 0 AND vendido_prop = 0 AND force_hide_prop != 1 ';

if (isset($_COOKIE['viewed-props']) && $_COOKIE['viewed-props'] != '') {

    $viewpropsquery = "WHERE id_prop IN (" . $_COOKIE['viewed-props'] . ") AND procesada_img = 1 AND activado_prop = 1 AND alquilado_prop = 0 AND vendido_prop = 0 AND force_hide_prop != 1";

    $viewedproperties = getRecords("
        SELECT
            id_prop,
            CASE WHEN properties_loc3.name_" . $lang . "_loc3 IS NOT NULL THEN properties_loc3.name_" . $lang . "_loc3 ELSE areas1.name_" . $lang . "_loc3  END AS area,
            CASE WHEN properties_types.types_" . $lang . "_typ IS NOT NULL THEN properties_types.types_" . $lang . "_typ ELSE types.types_" . $lang . "_typ END AS type,
            ...
            $viewpropsquery
        GROUP BY id_prop
        ORDER BY FIELD(id_prop, " . $_COOKIE['viewed-props'] . ")
        LIMIT 3
    ");
}

$smarty->assign("titleNewsSearch", $titleNewsSearch);
$smarty->assign("viewedproperties", $viewedproperties);
