...
$urlEL = ''; if(isset($urlStr['category']['el'])) $urlEL =  preg_replace('/\/'.$tokens[1].'\//', '/' . $urlStr['category']['el'] . '/', $urlEL);
...
$urlIT = ''; if(isset($urlStr['category']['it'])) $urlIT =  preg_replace('/\/'.$tokens[1].'\//', '/' . $urlStr['category']['it'] . '/', $urlIT);
$urlNL =  preg_replace('/\/'.$tokens[1].'\//', '/' . $urlStr['category']['nl'] . '/', $urlNL);
$urlNO =  preg_replace('/\/'.$tokens[1].'\//', '/' . $urlStr['category']['no'] . '/', $urlNO);
$urlPT = ''; if(isset($urlStr['category']['pt'])) $urlPT =  preg_replace('/\/'.$tokens[1].'\//', '/' . $urlStr['category']['pt'] . '/', $urlPT);
...
function replaceParet($parent) {
    ...
    global $languages, $language, $lang, $urlDefault, $urlCA, $urlDA, $urlDE, $urlEL, $urlEN, $urlES, $urlFI, $urlFR, $urlIS, $urlIT, $urlNL, $urlNO, $urlPT, $urlRU, $urlSE, $urlZH, $urlPL;
...
function GetChildsIds($par) {
    ...
    if ($par != '') {
        ...
        SELECT news_categories.id_ct
        FROM news_categories
        WHERE type_ct = 10  AND parent_ct = '" . simpleSanitize(($par)) . "'
        ORDER BY news_categories.orden_ct ASC
        ");
...
$_query = "
 ...
 AND news.content_".$lang."_nws != '' AND type_nws = 10
