...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    if (!class_exists('SmartyPaginate')) {
        trigger_error("paginate_middle: missing SmartyPaginate class");
        return;
    }
    if (!isset($_SESSION['SmartyPaginate'])) {
        trigger_error("paginate_middle: SmartyPaginate is not initialized, use connect() first");
        return;
    }
        ...
        switch($_key) {
            case 'id':
                if (!SmartyPaginate::isConnected($_val)) {
                    trigger_error("paginate_middle: unknown id '$_val'");
                    return;
                }
                $_id = $_val;
                break;
    ...
    if (!isset($_SESSION['SmartyPaginate'][$_id]['item_total'])) {
        trigger_error("paginate_middle: total was not set");
        return;
    }
    ...
    while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
            ...
            if ($_item < $_curr_item) {
                $_ret .= $_link_prefix . '<a href="' . str_replace('&', '&amp;', $_this_url) . '"' . $_attrs . ' rel="prev">' . $_text . '</a>' . $_link_suffix;
            } else {
                $_ret .= $_link_prefix . '<a href="' . str_replace('&', '&amp;', $_this_url) . '"' . $_attrs . ' rel="next">' . $_text . '</a>' . $_link_suffix;
            }
        } else {
            $_ret .= '<span class="btn btn-primary">' . $_link_prefix . $_text . $_link_suffix . '</span>';
        }
        $_item += $_limit;
        $_page++;
        $_display_pages++;
