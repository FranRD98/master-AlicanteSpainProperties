...
function smarty_function_paginate_next($params, &$smarty) {
    ...
    if (!class_exists('SmartyPaginate')) {
        trigger_error("paginate_next: missing SmartyPaginate class");
        return;
    }
    if (!isset($_SESSION['SmartyPaginate'])) {
        trigger_error("paginate_next: SmartyPaginate is not initialized, use connect() first");
        return;
    }
        ...
        switch($_key) {
            case 'id':
                if (!SmartyPaginate::isConnected($_val)) {
                    trigger_error("paginate_next: unknown id '$_val'");
                    return;
                }
                $_id = $_val;
                break;
    ...
    if (SmartyPaginate::getTotal($_id) === false) {
        trigger_error("paginate_next: total was not set");
        return;
    }
    ...
    return $_show ? '<a href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . ' rel="next">' . $_text . '</a>' : '';
}
