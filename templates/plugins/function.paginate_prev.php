...
function smarty_function_paginate_prev($params, $smarty) {
    ...
    if (!class_exists('SmartyPaginate')) {
        trigger_error("paginate_prev: missing SmartyPaginate class");
        return;
    }
    if (!isset($_SESSION['SmartyPaginate'])) {
        trigger_error("paginate_prev: SmartyPaginate is not initialized, use connect() first");
        return;
    }
        ...
        switch($_key) {
            case 'id':
                if (!SmartyPaginate::isConnected($_val)) {
                    trigger_error("paginate_prev: unknown id '$_val'");
                    return;
                }
                $_id = $_val;
                break;
    ...
    if (SmartyPaginate::getTotal($_id) === false) {
        trigger_error("paginate_prev: total was not set");
        return;
    }
    ...
    return $_show ? '<a href="' . str_replace('&','&amp;', $_url) . '"' . $_attrs . ' rel="prev">' . $_text . '</a>' : '';
}
