 ...
 * File:        function.paginate_prev2.php
...
function smarty_function_paginate_prev2($params, &$smarty) {

    $_id = 'default';
    $_attrs = array();

    if (!class_exists('SmartyPaginate')) {
        trigger_error("paginate_prev2: missing SmartyPaginate class");
        return;
    }
    if (!isset($_SESSION['SmartyPaginate'])) {
        trigger_error("paginate_prev2: SmartyPaginate is not initialized, use connect() first");
        return;
    }
        ...
        switch($_key) {
            case 'id':
                if (!SmartyPaginate::isConnected($_val)) {
                    trigger_error("paginate_prev2: unknown id '$_val'");
                    return;
                }
                $_id = $_val;
                break;
    ...
    if (SmartyPaginate::getTotal($_id) === false) {
        trigger_error("paginate_prev2: total was not set");
        return;
    }
    ...
    return $_show ? '<link rel="prev" href="' . $_SERVER['REQUEST_SCHEME'] . '//'. $_SERVER['HTTP_HOST'] . str_replace('&','&amp;', $_url) . '" />' : '';
