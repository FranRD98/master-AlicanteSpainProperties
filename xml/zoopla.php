...
WHERE properties_properties.activado_prop = 1 AND properties_properties.export_zoopla_prop = 1 AND descripcion_en_prop != '' AND rightmove_tipo_prop != '' AND id_rml != '' AND vendido_prop = 0 AND vendido_tag_prop = 0 AND reservado_prop = 0 AND alquilado_prop = 0
...
if ($totalRows_rsProperties > 0) {
    ...
    $return .=  "#DEFINITION#\n";
    $return .=  "AGENT_REF|HOUSE_NAME_NUMBER|STREET_NAME|OS_TOWN_CITY|OS_REGION|ZIPCODE|COUNTRY_CODE|FEATURE1|FEATURE2|FEATURE3|FEATURE4|FEATURE5|FEATURE6|FEATURE7|FEATURE8|FEATURE9|FEATURE10|FEATURE11|FEATURE12|FEATURE13|FEATURE14|FEATURE15|FEATURE16|FEATURE17|FEATURE18|FEATURE19|FEATURE20|EXACT_LATITUDE|EXACT_LONGITUDE|SUMMARY|DESCRIPTION|CREATE_DATE|UPDATE_DATE|BRANCH_ID|STATUS_ID|BEDROOMS|PRICE|PRICE_QUALIFIER|NEW_HOME_FLAG|PROP_SUB_ID|DISPLAY_ADDRESS|PUBLISHED_FLAG|TRANS_TYPE_ID|MEDIA_IMAGE_00|MEDIA_IMAGE_01|MEDIA_IMAGE_02|MEDIA_IMAGE_03|MEDIA_IMAGE_04|MEDIA_IMAGE_05|MEDIA_IMAGE_06|MEDIA_IMAGE_07|MEDIA_IMAGE_08|MEDIA_IMAGE_09|MEDIA_IMAGE_10|MEDIA_IMAGE_11|MEDIA_IMAGE_12|MEDIA_IMAGE_13|MEDIA_IMAGE_14|MEDIA_IMAGE_15|MEDIA_IMAGE_16|MEDIA_IMAGE_17|MEDIA_IMAGE_18|MEDIA_IMAGE_19|MEDIA_FLOOR_PLAN_00|MEDIA_FLOOR_PLAN_01|MEDIA_FLOOR_PLAN_02|MEDIA_FLOOR_PLAN_03|MEDIA_FLOOR_PLAN_04|MEDIA_FLOOR_PLAN_05|MEDIA_FLOOR_PLAN_06|MEDIA_FLOOR_PLAN_07|MEDIA_FLOOR_PLAN_08|MEDIA_FLOOR_PLAN_09|MEDIA_VIRTUAL_TOUR_00|~\n\n";
    ...
    do {
        ...
        $return .= $zooplaAgentRef . '_' . str_replace(['.', ' '], '', $row_rsProperties['referencia_prop']). '|';  // Número de agente AGENT_REF
...
WHERE properties_property_feature.property = '".$row_rsProperties['id_prop']."' LIMIT 20";
        ...
        $numFeat =  20 - $totalRows_rsproperties_features;
        if ($numFeat > 0) {
            for ($i=1; $i <= $numFeat; $i++) {
...
if ($row_rsProperties['descripcion_xml_en_prop'] != '') {
    ...
    $return .= mysqli_real_escape_string($inmoconn,strip_tags(str_replace(["\r\n", "\r", "˚"], " ", $row_rsProperties['descripcion_xml_en_prop']))) . '|'; // Descripción DESCRIPTION
} else {
...
$return .= $row_rsProperties['inserted_xml_prop'] . '|'; // Creado CREATE_DATE
$return .= $row_rsProperties['updated_prop'] . '|'; // Actualizado UPDATE_DATE
$return .= $zooplaBranchId . '|'; // Branch ID BRANCH_ID
...
$return .= '1|'; // TRANS_TYPE_ID
        ...
        $query_rsImages = "SELECT * FROM properties_images WHERE property_img = '".$row_rsProperties['id_prop']."' ORDER BY order_img LIMIT 20";
        ...
        do {
            if ($row_rsImages['image_img'] != '') {
                ...
                $imgName = $zooplaAgentRef . '_' . str_replace(['.', ' ', '-'], '', $row_rsProperties['referencia_prop']). '_IMG_' . $num . '.' . $ext;
        ...
        do {
            ...
            $imgName = $zooplaAgentRef . '_' . str_replace(['.', ' ', '-'], '', $row_rsProperties['referencia_prop']). '_FLP_' . $num . '.' . $ext;
    ...
    $newfile = "zoopla/".$zooplaAgentRef.".blm";
    $file = fopen($newfile, "w");
    fwrite($file, $return);
    fclose($file);

    if (file_exists($zooplaAgentRef.".zip")) {
        unlink($zooplaAgentRef.".zip");
    }

    ini_set("max_execution_time", 0);
    $zip = new ZipArchive();
    if ($zip->open($zooplaAgentRef."_".date("Y")."".date("m")."".date("d")."01.zip", ZIPARCHIVE::CREATE) !== true) {
        die("Could not open archive");
    }

    $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($_SERVER["DOCUMENT_ROOT"] . '/xml/zoopla/'));
    ...
    foreach ($iterator as $key=>$value) {
        ...
        if ($filExtens == 'blm' || $filExtens == 'jpg' || $filExtens == 'jpeg' || $filExtens == 'gif' || $filExtens == 'JPG' || $filExtens == 'JPEG' || $filExtens == 'GIF' || $filExtens == 'png' || $filExtens == 'PNG') {
            $zip->addFile(realpath($key), str_replace($_SERVER["DOCUMENT_ROOT"] . '/xml/zoopla/', "", $key)) or die("ERROR: Could not add file: $key");
        }
    }

    $zip->close();

    array_map('unlink', glob($_SERVER["DOCUMENT_ROOT"] . "/xml/zoopla/*"));

    if (!isset($_GET['debug_error'])) {
        include "inc/SFTP.php";
        $ftp = new SFTP($zooplaFTP, $zooplaFTPuser, $zooplaFTPpass);
        if ($ftp->connect()) {
            // $ftp->cd('live/upload');
        ...
        header("Content-Disposition: attachment; filename=$zooplaAgentRef.'_".date("Y")."".date("m")."".date("d")."01.zip'");
        header('Content-Length: ' . filesize($zipname));
        header("Location: ".$zooplaAgentRef."_".date("Y")."".date("m")."".date("d")."01.zip");
    }
